...
class GlassBlur:
    ...
    def __call__(self, img, mag=-1, prob=1.):
        ...
        # img = np.uint8(gaussian(np.asarray(img) / 255., sigma=c[0], multichannel=True) * 255)  # deprecated in new skimage
        ...
        # img = np.clip(gaussian(img / 255., sigma=c[0], multichannel=True), 0, 1) * 255  # deprecated in new skimage
