...
class Rain:
    ...
    def __call__(self, img, mag=-1, prob=1.):
        ...
        n_channels = len(img.getbands())
        isgray = n_channels == 1
        line_width = self.rng.integers(3, 8)  # thicker lines

        # OLD: c = [50, 70, 90] - too sparse for small crops
        # NEW: heavier rain
        c = [100, 150, 200]
        if mag < 0 or mag >= len(c):
            index = 0
