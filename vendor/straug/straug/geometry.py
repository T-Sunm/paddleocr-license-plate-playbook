...
class Rotate:
    ...
    def __call__(self, img, iscurve=False, mag=-1, prob=1.):
        ...
        # NEW: fixed angles only (5° or -5°)
        fixed_angles = [5, 7, 10]  # mag 0, 1, 2
        if mag < 0 or mag >= len(fixed_angles):
            angle = fixed_angles[0]
        else:
            angle = fixed_angles[mag]
        # Random choose positive or negative (complement)
        if self.rng.uniform(0, 1) < 0.5:
            angle = -angle
