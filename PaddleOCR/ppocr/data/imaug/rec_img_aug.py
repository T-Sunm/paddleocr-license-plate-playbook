...
class RecAug(object):
    ...
    def __call__(self, data):
        ...
        if random.random() <= self.tia_prob:
            if h >= 20 and w >= 20:
                if len(img.shape) == 4:
                    # Apply same TIA to all frames
                    # Note: this might be slow if we don't optimize, but correct
                    # Ideally we want to generate params once and apply to all
                    # For now just loop
                    for t in range(T):
                        # To ensure same distortion, we should ideally peek into tia_distort
                        # But simpler is to set seeds if TIA uses random
                        # PaddleOCR's TIA uses random.randint/random.random internally
                        # Let's fix seed for each sample's TIA application
                        seed = random.randint(0, 1000000)
                        random.seed(seed)
                        img[t] = tia_distort(img[t], random.randint(3, 6))
                        random.seed(seed)
                        random.randint(3, 6) # skip distortion params
                        img[t] = tia_stretch(img[t], random.randint(3, 6))
                        random.seed(seed)
                        random.randint(3, 6); random.randint(3, 6) # skip
                        img[t] = tia_perspective(img[t])
                else:
...
class BaseDataAugmentation(object):
    ...
    def __call__(self, data):
        ...
        if random.random() <= self.blur_prob:
            ...
            if len(img.shape) == 4:
                for t in range(T):
                    img[t] = cv2.sepFilter2D(img[t], -1, self.fil, self.fil)
            else:
...
def resize_norm_img(img, image_shape, padding=True, interpolation=cv2.INTER_LINEAR):
    ...
    if not padding:
        if is_4d:
            resized_imgs = []
            for t in range(T):
                resized_imgs.append(cv2.resize(img[t], (imgW, imgH), interpolation=interpolation))
            resized_image = np.stack(resized_imgs, axis=0)
        else:
        ...
        if is_4d:
            resized_imgs = []
            for t in range(T):
                resized_imgs.append(cv2.resize(img[t], (resized_w, imgH)))
            resized_image = np.stack(resized_imgs, axis=0)
        else:
            resized_image = cv2.resize(img, (resized_w, imgH))
    ...
    if is_4d:
        # [T, H, W, C] -> [T, C, H, W]
        resized_image = resized_image.transpose((0, 3, 1, 2)) / 255
        resized_image -= 0.5
        resized_image /= 0.5
        padding_im = np.zeros((T, imgC, imgH, imgW), dtype=np.float32)
        padding_im[:, :, :, 0:resized_w] = resized_image
    else:
        # [H, W, C] -> [C, H, W]
        if len(resized_image.shape) == 2: # Gray
            resized_image = resized_image[np.newaxis, :]
        else:
            resized_image = resized_image.transpose((2, 0, 1)) / 255
        resized_image -= 0.5
...
def hsv_aug(img):
    ...
    if len(img.shape) == 4:
        ...
        for t in range(T):
            ...
            hsv[:, :, 2] = hsv[:, :, 2] * (1 + delta)
            img[t] = cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR)
        return img
    else:
