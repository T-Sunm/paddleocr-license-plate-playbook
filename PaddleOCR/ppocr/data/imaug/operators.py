...
class DecodeMultiImage(object):
    """decode multiple images and stack them with padding to max size"""
    ...
    def __call__(self, data):
        ...
        for img in img_list:
            ...
            ), "invalid input 'img' in DecodeMultiImage"
            img_np = np.frombuffer(img, dtype="uint8")

            flag = cv2.IMREAD_GRAYSCALE if self.img_mode == "GRAY" else cv2.IMREAD_COLOR
            if self.ignore_orientation:
                flag |= cv2.IMREAD_IGNORE_ORIENTATION

            img_decoded = cv2.imdecode(img_np, flag)
            if img_decoded is None:
                return None

            if self.img_mode == "GRAY":
                img_decoded = cv2.cvtColor(img_decoded, cv2.COLOR_GRAY2BGR)
            elif self.img_mode == "RGB":
                img_decoded = img_decoded[:, :, ::-1]  # BGR -> RGB

            decoded_imgs.append(img_decoded)

        # Pad all frames to the same maximum shape (H, W)
        hs = [im.shape[0] for im in decoded_imgs]
        ws = [im.shape[1] for im in decoded_imgs]
        H, W = max(hs), max(ws)
        ...
        # Stack images: [T, H, W, C] or [T, C, H, W]
        data["image"] = np.stack(padded, axis=0)
        return data
...
class NormalizeImage(object):
    ...
    def __call__(self, data):
        ...
        if is_4d:
            # Broadcast mean/std to [T, C, 1, 1] or [1, 1, 1, C]
            # Assuming mean/std already shaped as (3, 1, 1) or (1, 1, 3)
            # If [T, H, W, C] and mean is (1, 1, 3), it broadcasts naturally.
            # If [T, C, H, W] and mean is (3, 1, 1), it needs to be (1, 3, 1, 1).
            if self.mean.shape == (3, 1, 1):
