...
class SimpleDataSet(Dataset):
    ...
    def _load_image(self, file_name_info):
        if isinstance(file_name_info, (list, dict)):
            if isinstance(file_name_info, dict):
                if "hr" in file_name_info:
                    paths = file_name_info["hr"]
                elif "lr" in file_name_info:
                    paths = file_name_info["lr"]
                else:
                    paths = []
            else:
    ...
    def get_ext_data(self):
        ...
        while len(ext_data) < ext_data_num:
            ...
            file_name_info = self._try_parse_filename_list(file_name)
            img, img_path = self._load_image(file_name_info)
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            img, img_path = self._load_image(file_name_info)
            if img is None:
                raise Exception("Failed to load image: {}".format(file_name))
    ...
    def resize_norm_img(self, data, imgW, imgH, padding=True):
        ...
        if not padding:
            if is_4d:
                ...
                resized_imgs = []
                for t in range(T):
                    resized_imgs.append(cv2.resize(img[t], (imgW, imgH), interpolation=cv2.INTER_LINEAR))
