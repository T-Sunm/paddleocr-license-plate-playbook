...
class PPLCNetV3(nn.Layer):
    ...
    def freeze_stages(self):
        ...
        # Freeze Stem (conv1) corresponds to stage 1 approx or stage 0
        if hasattr(self, 'conv1'):
             self.freeze_parameters(self.conv1)
        
        # Freeze Blocks
        if self.frozen_stages >= 2:
            if hasattr(self, 'blocks2'):
                self.freeze_parameters(self.blocks2)
                
        if self.frozen_stages >= 3:
            if hasattr(self, 'blocks3'):
                self.freeze_parameters(self.blocks3)
                
        if self.frozen_stages >= 4:
             if hasattr(self, 'blocks4'):
                self.freeze_parameters(self.blocks4)
        
        if self.frozen_stages >= 5:
             if hasattr(self, 'blocks5'):
                self.freeze_parameters(self.blocks5)
