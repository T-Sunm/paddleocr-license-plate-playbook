...
from .attn_blocks import SEBlock, CBAM, SpatialAttention
...
class PPHGNetV2(TheseusLayer):
    ...
    def __init__(
        ...
        text_rec=False,
        out_indices=None,
        use_se=False,           # Disable for distillation
        se_reduction=16,
        use_cbam=False,         # Disable for distillation
        cbam_reduction=16,
        cbam_spatial_kernel=7,
        use_spatial_attn=False, # Disable for distillation
        use_temporal=False,     
        frozen_stages=-1,       # Added from kwargs to fix NameError
        **kwargs,
    ):
        super().__init__()
        self.det = det
        ...
        # --- Simplify: Disable Attention for Distillation Training ---
        self.use_se = use_se
        self.use_cbam = use_cbam
        self.use_spatial_attn = use_spatial_attn
        self.use_temporal = use_temporal

        # last_stage_name = list(stage_config.keys())[-1]
        # final_channels = stage_config[last_stage_name][2] 
        
        # if self.use_se:
        #     self.se = SEBlock(final_channels, reduction=se_reduction)
        # if self.use_cbam:
        #     self.cbam = CBAM(final_channels, reduction=cbam_reduction, spatial_kernel=cbam_spatial_kernel)
        # if self.use_spatial_attn:
        #     self.sa = SpatialAttention(kernel_size=cbam_spatial_kernel)
        ...
        for i, k in enumerate(stage_config):
            ...
            # --- Only use Stage 1 and Stage 2 ---
            if i >= 2:
                # We still keep them in list to avoid index errors, but they won't be used in forward if temporal
                stage = HGV2_Stage(
                    in_channels, mid_channels, out_channels, block_num,
                    layer_num, is_downsample, light_block, kernel_size,
                    use_lab, stride, lr_mult=lr_mult_list[i + 1],
                )
            ...
            if self.use_temporal:
                # Use base stage (2D), expansion to handle 5D happens in forward
                self.stages.append(stage_2d)
                
                # Create corresponding T-Block
                if i == 0: # Stage 1 (Index 0) -> Level 1 Fusion
                    # PPHGNetV2_B4 Stage 1 uses stride [2, 1] for rec (reduction H)
                    t_block = nn.Sequential(
                        ResBlock3D_Paddle(48, 128, 48, stride=(1, 2, 1)),
                        ResBlock3D_Paddle(128, 128, 48)
                    )
                else: # Stage 2 (Index 1) -> Level 2 Fusion
                    # PPHGNetV2_B4 Stage 2 uses stride [1, 2] for rec (reduction W)
                    t_block = nn.Sequential(
                        ResBlock3D_Paddle(128, 512, 128, stride=(1, 1, 2)),
        ...
        if self.use_temporal:
            ...
            # Override out_channels for the model
        ...
        # --- Comment out Stage 3-4 components if using temporal adapter ---
        if not self.use_temporal:
    ...
    def forward(self, x):
        if self.use_temporal:
            ...
            # --- TimeDistributed Logic for Stem ---
            # Input: [B, C, T, H, W] -> Flatten for 2D stem
            x_flat = x.transpose([0, 2, 1, 3, 4]).reshape([B * T, C, H, W])
            s_x = self.stem(x_flat)
            
            # Reshape back to 5D: [B*T, C', H', W'] -> [B, C', T, H', W']
            s_shape = s_x.shape
            s_x = s_x.reshape([B, T, s_shape[1], s_shape[2], s_shape[3]]).transpose([0, 2, 1, 3, 4])
            
            # Branch Temporal
            t_x = self.temporal_stem(x)
        else:
            s_x = self.stem(x)

        out = []
        for i, stage in enumerate(self.stages):
            # Với Hybrid Temporal, chỉ xài đến Stage 2 (Index 1)
            if self.use_temporal and i >= 2:
                break
                
            if self.use_temporal:
                # --- TimeDistributed Logic for Stage ---
                B, C, T, H, W = s_x.shape
                s_x_flat = s_x.transpose([0, 2, 1, 3, 4]).reshape([B * T, C, H, W])
                s_x_flat = stage(s_x_flat)
                s_shape = s_x_flat.shape
                s_x = s_x_flat.reshape([B, T, s_shape[1], s_shape[2], s_shape[3]]).transpose([0, 2, 1, 3, 4])
                
                # Branch Temporal Fusion
                t_x = self.temporal_stages[i](t_x)
                s_x = s_x + t_x
            else:
                s_x = stage(s_x)
        ...
        if self.use_temporal:
            # Temporal Aggregation (TAM)
            x_2d = self.tam(s_x) # [B, 1024, H', W']
            # Neck Alignment (H=1)
            return self.adapter(x_2d)

        # --- Original Logic for Recognition/Detection ---
        if self.det:
            return out

        if self.text_rec:
            # --- Simplify: Comment out Attention Mechanisms for Distillation ---
            # if self.use_se:
            #     s_x = self.se(s_x)
            # if self.use_cbam:
            #     s_x = self.cbam(s_x)
            ...
            if self.training:
                s_x = F.adaptive_avg_pool2d(s_x, [1, 40])
            else:
                s_x = F.avg_pool2d(s_x, [3, 2])
        return s_x
