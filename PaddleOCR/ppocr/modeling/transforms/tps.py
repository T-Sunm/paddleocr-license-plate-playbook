...
class ConvBNLayer(nn.Layer):
    def __init__(
        ...
        self.conv = nn.Conv2D(
            ...
            groups=groups,
            weight_attr=ParamAttr(name=full_name + "_weights"),
            bias_attr=False,
        )
        bn_name = "bn_" + full_name
...
class LocalizationNetwork(nn.Layer):
    def __init__(self, in_channels, num_fiducial, loc_lr, model_name, name_prefix=""):
        ...
        self.fc1 = nn.Linear(
            ...
            weight_attr=ParamAttr(
                ...
                name=name_prefix + name + "_w",
                initializer=nn.initializer.Uniform(-stdv, stdv),
            ),
            bias_attr=ParamAttr(name=name_prefix + name + ".b_0"),
        ...
        bias_attr = ParamAttr(
            ...
            name=name_prefix + name + "_b",
        )
        self.fc2 = nn.Linear(
            fc_dim, F * 2, weight_attr=param_attr, bias_attr=bias_attr, name=name_prefix + name
        )
...
class GridGenerator(nn.Layer):
    def __init__(self, in_channels, num_fiducial, name_prefix=""):
        ...
        param_attr = ParamAttr(
            learning_rate=0.0, initializer=initializer, name=name_prefix + name + "_w"
        )
        bias_attr = ParamAttr(
            learning_rate=0.0, initializer=initializer, name=name_prefix + name + "_b"
        )
        self.fc = nn.Linear(
            in_channels, 6, weight_attr=param_attr, bias_attr=bias_attr, name=name_prefix + name
        )
    ...
    def build_P_paddle(self, I_r_size):
        ...
        I_r_grid_x = paddle.divide(
            paddle.arange(-I_r_width, I_r_width, 2, dtype="float64") + 1.0,
            paddle.to_tensor(I_r_width, dtype="float64"),
        )
        ...
        I_r_grid_y = paddle.divide(
            paddle.arange(-I_r_height, I_r_height, 2, dtype="float64") + 1.0,
            paddle.to_tensor(I_r_height, dtype="float64"),
        )
...
class TPS(nn.Layer):
    def __init__(self, in_channels, num_fiducial, loc_lr, model_name, name_prefix=""):
        ...
        self.loc_net = LocalizationNetwork(
            in_channels, num_fiducial, loc_lr, model_name, name_prefix=name_prefix
        )
        self.grid_generator = GridGenerator(self.loc_net.out_channels, num_fiducial, name_prefix=name_prefix)
    ...
    def forward(self, image):
        ...
        if len(image.shape) == 5:
            ...
            for t in range(T):
                frame = image[:, t, :, :, :]
                batch_C_prime = self.loc_net(frame)
                batch_P_prime = self.grid_generator(batch_C_prime, frame.shape[2:])
                batch_P_prime = batch_P_prime.reshape([-1, H, W, 2])
                is_fp16 = False
                if batch_P_prime.dtype != paddle.float32:
                    data_type = batch_P_prime.dtype
                    frame = frame.cast(paddle.float32)
                    batch_P_prime = batch_P_prime.cast(paddle.float32)
                    is_fp16 = True
                batch_I_r = F.grid_sample(x=frame, grid=batch_P_prime)
                if is_fp16:
                    batch_I_r = batch_I_r.cast(data_type)
                outputs.append(batch_I_r)
